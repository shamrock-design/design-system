...
You are building UI for an app in the Shamrock ecosystem. **All UI must be built from `@shamrock/ui`, `@shamrock/tokens`, and `@shamrock/icons`.** Do not hand-roll styled divs, do not use Tailwind classes, do not invent colors.

## Setup (once per app)

```tsx
import "@shamrock/tokens/css/core.css";
import "@shamrock/tokens/css/theme-clover.css"; // the app's brand theme
...
9. Icons: `<Icon name="..." size={16} />` from `@shamrock/icons` (names in `ICON_NAMES`). Never inline foreign SVG icon sets.
10. Assets (illustrations/lottie): import from `@shamrock/assets/<category>/<name>` — check its `manifest.json` for what exists. Never copy asset files into the app.

## Page skeleton

```tsx
import { Aurora, Stack, Inline, Grid, Text } from "@shamrock/ui";
...
Compose it from the primitives + semantic tokens following the rules above, and flag it as a candidate for promotion into `@shamrock/ui`. Do not import another component library to fill the gap.
